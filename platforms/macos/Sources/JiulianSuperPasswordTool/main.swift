...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func buildWindow() {
        ...
        for (name, field, h) in [("光猫 IP：", hostField, "例如 192.168.0.1"), ("登录端口：", portField, "默认 23"), ("登录用户名：", userField, "例如 root"), ("登录密码：", passField, "明文显示"), ("保存目录：", outField, "") ] {
            v.addSubview(label(name, x: x1, y: y)); setupField(field, x: x2, y: y, w: name == "保存目录：" ? 500 : 390); v.addSubview(field); if !h.isEmpty { v.addSubview(hint(h, x: x3, y: y)) }; y -= 35
        }
        chooseButton.frame = NSRect(x: 675, y: 370, width: 78, height: 30); chooseButton.target = self; chooseButton.action = #selector(chooseDir); v.addSubview(chooseButton)
        cleanBox.frame = NSRect(x: x2, y: 335, width: 300, height: 24); cleanBox.state = cleanBox.state == .off ? .on : cleanBox.state; v.addSubview(cleanBox)

        startButton.frame = NSRect(x: x2, y: 290, width: 160, height: 34); startButton.bezelStyle = .rounded; startButton.target = self; startButton.action = #selector(start); v.addSubview(startButton)
        clearButton.frame = NSRect(x: 340, y: 290, width: 90, height: 34); clearButton.target = self; clearButton.action = #selector(clearLogs); v.addSubview(clearButton)
        statusLabel.frame = NSRect(x: 485, y: 290, width: 260, height: 34); statusLabel.font = .boldSystemFont(ofSize: 22); statusLabel.textColor = .systemBlue; v.addSubview(statusLabel)

        let box = NSBox(frame: NSRect(x: 30, y: 245, width: 820, height: 90)); box.title = "解密结果"; v.addSubview(box)
        accountLabel.frame = NSRect(x: 145, y: 295, width: 650, height: 22); passwordLabel.frame = NSRect(x: 145, y: 270, width: 650, height: 22); outputLabel.frame = NSRect(x: 145, y: 245, width: 650, height: 22)
        for (t, yy) in [("超级账号：", 295 as CGFloat), ("超级密码：", 270 as CGFloat), ("保存位置：", 245 as CGFloat)] { v.addSubview(label(t, x: 40, y: yy, w: 95)) }
        for l in [accountLabel, passwordLabel, outputLabel] { l.isSelectable = true; v.addSubview(l) }

        let logBox = NSBox(frame: NSRect(x: 30, y: 20, width: 820, height: 215)); logBox.title = "运行日志"; v.addSubview(logBox)
        let scroll = NSScrollView(frame: NSRect(x: 42, y: 33, width: 796, height: 182)); scroll.hasVerticalScroller = true; logView.isEditable = false; logView.font = .monospacedSystemFont(ofSize: 12, weight: .regular); logView.backgroundColor = .textBackgroundColor; scroll.documentView = logView; v.addSubview(scroll)
