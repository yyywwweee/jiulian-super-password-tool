...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func buildWindow() {
        ...
        let topMask: NSView.AutoresizingMask = [.minYMargin]
        let title = NSTextField(labelWithString: "\(AppConstants.appName) \(AppVersion.display)"); title.frame = NSRect(x: 28, y: 595, width: 700, height: 34); title.font = .boldSystemFont(ofSize: 28); title.autoresizingMask = topMask; v.addSubview(title)
        let sub = NSTextField(labelWithString: "本工具只获取超级管理员账号和密码。登录密码明文显示，并会缓存在本机，便于下次自动填入。"); sub.frame = NSRect(x: 30, y: 568, width: 760, height: 22); sub.textColor = .secondaryLabelColor; sub.autoresizingMask = topMask; v.addSubview(sub)
        let versionLabel = NSTextField(labelWithString: "版本：\(AppVersion.detail)"); versionLabel.frame = NSRect(x: 30, y: 548, width: 760, height: 20); versionLabel.textColor = .tertiaryLabelColor; versionLabel.font = .systemFont(ofSize: 12); versionLabel.autoresizingMask = topMask; v.addSubview(versionLabel)
        ...
        for (name, field, h) in [("光猫 IP：", hostField, "例如 192.168.0.1"), ("登录端口：", portField, "默认 23"), ("登录用户名：", userField, "例如 root"), ("登录密码：", passField, "明文显示"), ("保存目录：", outField, "") ] {
            let lbl = label(name, x: x1, y: y); lbl.autoresizingMask = topMask; v.addSubview(lbl)
            setupField(field, x: x2, y: y, w: name == "保存目录：" ? 500 : 390); field.autoresizingMask = topMask; v.addSubview(field)
            if !h.isEmpty { let hh = hint(h, x: x3, y: y); hh.autoresizingMask = topMask; v.addSubview(hh) }
            y -= 39
        }
        chooseButton.frame = NSRect(x: 675, y: 354, width: 78, height: 30); chooseButton.target = self; chooseButton.action = #selector(chooseDir); chooseButton.autoresizingMask = topMask; v.addSubview(chooseButton)
        cleanBox.frame = NSRect(x: x2, y: 315, width: 300, height: 24); cleanBox.state = cleanBox.state == .off ? .on : cleanBox.state; cleanBox.autoresizingMask = topMask; v.addSubview(cleanBox)

        startButton.frame = NSRect(x: x2, y: 271, width: 160, height: 34); startButton.bezelStyle = .rounded; startButton.target = self; startButton.action = #selector(start); startButton.autoresizingMask = topMask; v.addSubview(startButton)
        clearButton.frame = NSRect(x: 340, y: 271, width: 90, height: 34); clearButton.target = self; clearButton.action = #selector(clearLogs); clearButton.autoresizingMask = topMask; v.addSubview(clearButton)
        statusLabel.frame = NSRect(x: 485, y: 271, width: 260, height: 34); statusLabel.font = .boldSystemFont(ofSize: 22); statusLabel.textColor = .systemBlue; statusLabel.autoresizingMask = topMask; v.addSubview(statusLabel)

        let box = NSBox(frame: NSRect(x: 30, y: 150, width: 820, height: 105)); box.title = "解密结果"; box.autoresizingMask = topMask; v.addSubview(box)
        ...
        for (t, yy) in [("超级账号：", 217 as CGFloat), ("超级密码：", 190 as CGFloat), ("保存位置：", 163 as CGFloat)] { let lbl = label(t, x: 40, y: yy, w: 95); lbl.autoresizingMask = topMask; v.addSubview(lbl) }
        for l in [accountLabel, passwordLabel, outputLabel] { l.isSelectable = true; l.autoresizingMask = topMask; v.addSubview(l) }
