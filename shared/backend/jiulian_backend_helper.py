...
def cleanup_current_remote_tmp(tn, logs, remote_tmp, wait=0.2, timeout=3.0, final=False):
    """Check and clean only the temp file generated by this run.
    final=False: 本次清理失败后还有重试机会，不要说“请手动登录"""
    def _fail(msg):
        if final:
...
def run(params):
    ...
    try:
        ...
        if clean_tmp:
            if cleanup_current_remote_tmp(tn, logs, remote_tmp, wait=0.5, timeout=4.0, final=True):
                remote_tmp = ""
        else:
        ...
        if tn is not None:
            try:
                if not step5_done:
                    if clean_tmp:
                        cleaned = cleanup_current_remote_tmp(tn, logs, remote_tmp, wait=0.2, timeout=2.0, final=False)
                        if not cleaned:
                            # 当前 Telnet 可能已断开。
                            # 网络阻塞后恢复需要时间，不能立刻重连。
                            try:
                            ...
                            for retry in range(1, 4):
                                ...
                                log(logs, f"步骤 5/5：等待 {wait_sec} 秒后重连并清理（第 {retry}/3 次）…")
                                time.sleep(wait_sec)
                                tn2 = None
                                try:
                                    ...
                                    time.sleep(0.2)
                                    read_some(tn2, quiet=0.5, hard=1.5)
                                    if cleanup_current_remote_tmp(tn2, logs, remote_tmp, wait=0.2, timeout=2.0, final=False):
                                        reconnect_ok = True
                                    tn2.write(b"exit\n")
                                    tn2.close()
                                    if reconnect_ok:
